...
export function NodeDetail({ nodeId }: NodeDetailProps) {
  ...
  const {
    ...
    error,
  } = trpc.nodes.byId.useQuery(nodeId, {
  ...
  const { data: metrics } = trpc.nodes.metrics.useQuery(
    {
      nodeId,
      range: timeRange,
      aggregation: timeRange === "1h" ? "raw" : "hourly",
    },
    { refetchInterval: 60000 },
  );

  const { data: peers, isLoading: peersLoading } =
    trpc.nodes.peers.useQuery(nodeId);
    ...
    return (
      <div className="card p-12 text-center">
        <div className="w-16 h-16 rounded-full bg-status-inactive/10 mx-auto mb-4 flex items-center justify-center">
          <svg
            className="w-8 h-8 text-status-inactive"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
  ...
  const ramPercent =
    latestMetric?.ramUsed && latestMetric?.ramTotal
      ? Number(
          (BigInt(latestMetric.ramUsed) * BigInt(100)) /
            BigInt(latestMetric.ramTotal),
        )
      : 0;
  ...
  const ramChartData =
    metrics?.map((m) => ({
      time: "bucket" in m ? m.bucket : m.time,
      value:
        "avg_ram_percent" in m
          ? (m.avg_ram_percent ?? 0)
          : m.ramUsed && m.ramTotal
            ? Number((BigInt(m.ramUsed) * BigInt(100)) / BigInt(m.ramTotal))
            : 0,
    })) || [];
    ...
    <div className="space-y-8">
        ...
        <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <button
                onClick={() =>
                  navigator.clipboard.writeText(formatAddress(node.address))
                }
            ...
            <div className="flex flex-wrap gap-2">
              <span
                className={`status-badge status-badge-${node.isActive ? "active" : "inactive"}`}
              >
          ...
          <div className="text-sm text-muted-foreground">
            <div>
              First seen: {formatRelativeTime(new Date(node.firstSeen))}
            </div>
            {node.lastSeen && (
              <div>
      ...
      {latestMetric && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div className="card p-6 flex flex-col items-center">
            <MetricGauge value={ramPercent} label="RAM Usage" size="md" />
          </div>
          <div className="card p-6 flex flex-col items-center justify-center text-center">
            <div className="text-3xl font-bold">
              {formatBytes(latestMetric.fileSize ?? 0)}
            </div>
          ...
          <div className="card p-6 flex flex-col items-center justify-center text-center">
            <div className="text-3xl font-bold">
              {formatUptime(latestMetric.uptime ?? 0)}
            </div>
      ...
      {addressHistory && addressHistory.length > 0 && (
        <div className="card p-6">
          ...
          <p className="text-sm text-muted-foreground mb-4">
            This node has changed IP addresses {addressHistory.length} time
            {addressHistory.length > 1 ? "s" : ""}. The pubkey remains constant
            across IP changes.
          </p>
              ...
              >
                <div className="flex items-center gap-2 font-mono">
                  ...
                  <span className="text-foreground font-medium">
                    {formatAddress(change.newAddress)}
                  </span>
                </div>
