...
const ENDPOINTS: Endpoint[] = [
  {
    ...
    description:
      "Returns high-level network statistics including node counts, version distribution, and aggregate metrics.",
    response: {
      example: {
        nodes: { total: 45, active: 42, inactive: 3 },
        versions: [
          { version: "0.6.0", count: 30 },
          { version: "0.5.1", count: 15 },
        ],
        metrics: {
  ...
  },
  {
    ...
    description:
      "Returns detailed aggregate metrics with percentiles (p50, p90, p99) for CPU and RAM usage.",
    response: {
      example: {
  ...
  },
  {
    ...
    summary: "List Nodes",
    description:
      "Returns a paginated list of all known pNodes with optional filtering and sorting.",
    parameters: [
      {
        name: "status",
        in: "query",
      ...
      },
      {
        ...
        required: false,
        type: "string",
        description: "Search by address or pubkey",
      },
      {
        name: "limit",
        in: "query",
  ...
  },
  {
    ...
    description:
      "Returns detailed information about a specific node including latest metrics.",
    parameters: [
      {
        name: "id",
        in: "path",
        required: true,
        type: "string",
        description: "Node ID or address",
      },
    ],
    response: {
      example: {
  ...
  },
  {
    ...
    description:
      "Returns historical metrics for a node with configurable time range and aggregation.",
    parameters: [
      {
        name: "id",
        in: "path",
      ...
      },
      {
        ...
        required: false,
        type: "string",
        description: "Aggregation: raw, hourly, daily",
        default: "hourly",
      },
    ],
    response: {
    ...
    parameters: [
      {
        ...
        required: false,
        type: "string",
        description: "Ranking metric: uptime, cpu, ram, storage",
        default: "uptime",
      },
      {
        name: "order",
...
function EndpointCard({ endpoint }: { endpoint: Endpoint }) {
  ...
  return (
    <div className="border border-border rounded-xl overflow-hidden">
      ...
      >
        <span
          className={`px-2 py-1 text-xs font-mono rounded ${METHOD_COLORS[endpoint.method]}`}
        >
          {endpoint.method}
        </span>
                  ...
                  <div className="grid gap-3">
                    {endpoint.parameters.map((param) => (
                      <div key={param.name} className="flex items-center gap-2">
                        <label className="w-24 text-sm font-mono">
                          {param.name}
                        </label>
                        <input
                          type="text"
    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <section className="mb-8 p-6 border border-border rounded-xl">
        ...
        <p className="text-sm text-muted-foreground mb-4">
          API requests can be made anonymously (30 req/min) or with an API key
          for higher limits. Create API keys in your{" "}
          <Link
      ...
      <section className="mb-8 p-6 border border-border rounded-xl">
        ...
        <p className="text-sm text-muted-foreground mb-4">
          Rate limits are applied per-minute using a sliding window. Response
          headers include:
        </p>
