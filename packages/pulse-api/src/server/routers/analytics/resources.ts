...
export const resourcesRouter = createTRPCRouter({
  ...
  nodeRecommendations: publicProcedure
    ...
    .query(async ({ ctx, input: nodeId }) => {
      ...
      const resourceMetrics: NodeResourceMetrics = {
        ...
        ramPercent:
          latestMetric && Number(latestMetric.ramTotal) > 0
            ? (Number(latestMetric.ramUsed) / Number(latestMetric.ramTotal)) *
              100
            : 0,
  ...
  networkRecommendations: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      // Build lookup maps
      const metricsMap = new Map(latestMetrics.map((m) => [m.node_id, m]));
      const peerMap = new Map(peerCounts.map((p) => [p.nodeId, p._count.id]));
      ...
      const allRecommendations = nodes.map((node) => {
        ...
        const resourceMetrics: NodeResourceMetrics = {
          ...
          ramPercent:
            metric && Number(metric.ram_total) > 0
        ...
        return {
          nodeId: node.id,
          recommendations: generateTuningRecommendations(
            resourceMetrics,
            latestVersion,
          ),
        };
      ...
      const nodesWithIssues = allRecommendations
        ...
        .map((n) => {
          ...
          const criticalCount = n.recommendations.filter(
            (r) => r.priority === "critical",
          ).length;
          const highCount = n.recommendations.filter(
            (r) => r.priority === "high",
          ).length;
          return {
            nodeId: n.nodeId,
