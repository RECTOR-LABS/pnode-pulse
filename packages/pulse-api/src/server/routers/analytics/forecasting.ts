...
export const forecastingRouter = createTRPCRouter({
  ...
  networkForecast: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const storageHistory = await ctx.db.$queryRaw<
        Array<{
          bucket: Date;
      ...
      return {
        ...
        storage: storageForecast
          ? {
              ...
              currentValueGB:
                storageForecast.currentValue / (1024 * 1024 * 1024),
              predictionsGB: {
                nextWeek:
                  storageForecast.predictions.nextWeek / (1024 * 1024 * 1024),
                nextMonth:
                  storageForecast.predictions.nextMonth / (1024 * 1024 * 1024),
                next3Months:
                  storageForecast.predictions.next3Months /
                  (1024 * 1024 * 1024),
                next6Months:
                  storageForecast.predictions.next6Months /
                  (1024 * 1024 * 1024),
              },
            }
          : null,
