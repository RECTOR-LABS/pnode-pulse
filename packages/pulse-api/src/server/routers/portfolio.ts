      ...
      let portfolio = await ctx.db.portfolio.findFirst({
        ...
        include: {
          nodes: {
            ...
            orderBy: [{ isStarred: "desc" }, { addedAt: "desc" }],
          },
        },
      });
    ...
    .query(async ({ ctx, input }) => {
      ...
      for (const node of allNodes) {
        ...
        if (m) {
          ...
          if (m.ramUsed !== null && m.ramTotal !== null && m.ramTotal > 0) {
            networkMetrics.ram.push(
              (Number(m.ramUsed) / Number(m.ramTotal)) * 100,
            );
          }
          if (m.fileSize !== null)
            networkMetrics.storage.push(Number(m.fileSize));
      ...
      for (const pn of portfolio.nodes) {
        ...
        if (m) {
          ...
          if (m.ramUsed !== null && m.ramTotal !== null && m.ramTotal > 0) {
            portfolioMetrics.ram.push(
              (Number(m.ramUsed) / Number(m.ramTotal)) * 100,
            );
          }
          if (m.fileSize !== null)
            portfolioMetrics.storage.push(Number(m.fileSize));
      ...
      return {
        ...
        storage: {
          ...
          percentile: percentile(
            avg(portfolioMetrics.storage),
            networkMetrics.storage,
            false,
    ...
    .query(async ({ ctx, input }) => {
      ...
      for (const pn of portfolioNodes) {
        ...
        for (const event of nodeEvents) {
          if (event.eventType === "OFFLINE") {
            ...
            const duration = Math.round(
              (event.timestamp.getTime() - lastOffline.getTime()) / 60000,
            );
        ...
        if (lastOffline || !pn.node.isActive) {
          ...
          const duration = Math.round(
            (now.getTime() - offlineStart.getTime()) / 60000,
          );
          downtimeMinutes += duration;
          incidents.push({
            start: offlineStart,
      ...
      // Aggregate stats
      const totalDowntime = nodeStats.reduce(
        (sum, n) => sum + n.downtimeMinutes,
        0,
      );
      const avgDowntime =
        nodeStats.length > 0 ? totalDowntime / nodeStats.length : 0;
      const avgUptime =
        totalMinutes > 0
