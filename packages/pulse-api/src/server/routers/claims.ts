    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (input.verificationMethod === "WALLET_SIGNATURE" && !node.pubkey) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message:
            "Node does not have a registered public key. Use a different verification method.",
        });
      }
      ...
      const verificationData: Record<string, unknown> = {
        token,
        expiresAt: new Date(
          Date.now() + VERIFICATION_VALIDITY_MS,
        ).toISOString(),
      };

      // Add method-specific data
      if (input.verificationMethod === "WALLET_SIGNATURE") {
        verificationData.message = generateClaimMessage(
          node.address,
          ctx.walletAddress!,
          token,
        );
      ...
      switch (claim.verificationMethod) {
        ...
        case "VERIFICATION_FILE": {
          ...
          if (isPrivateIP(host)) {
            throw new TRPCError({
              code: "BAD_REQUEST",
              message:
                "Cannot verify nodes with private IP addresses. Node must have a public IP.",
            });
          }
          ...
          try {
            ...
            throw new TRPCError({
              code: "BAD_REQUEST",
              message:
                "Could not fetch verification file from node. Ensure the file is accessible at /.pnode-pulse-verify",
            });
          }
          break;
        }
        ...
        case "DNS_TXT": {
          ...
          throw new TRPCError({
            code: "BAD_REQUEST",
            message:
              "DNS verification is not yet fully implemented. Please use another method.",
          });
        }
      }
