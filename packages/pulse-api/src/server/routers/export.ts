...
export const exportRouter = createTRPCRouter({
      ...
      if (input.type === "current") {
        ...
        if (input.aggregation === "raw") {
          const metrics = await ctx.db.nodeMetric.findMany({
            ...
            include: {
              node: {
                select: { address: true, version: true, isActive: true },
              },
            },
            orderBy: { time: "asc" },
          ...
          for (const m of metrics) {
            const row = input.columns.map((col) => {
              switch (col) {
                ...
                case "ram_percent":
                  return m.ramUsed && m.ramTotal && m.ramTotal > BigInt(0)
                    ? ((Number(m.ramUsed) / Number(m.ramTotal)) * 100).toFixed(
                        2,
                      )
                    : "";
                case "ram_used":
                ...
                case "storage_gb":
                  return m.fileSize
                    ? (Number(m.fileSize) / 1e9).toFixed(2)
                    : "";
                case "uptime_hours":
                  return m.uptime ? (m.uptime / 3600).toFixed(2) : "";
                case "packets_received":
          ...
          const aggregatedData = await ctx.db.$queryRaw<
            Array<{
              bucket: Date;
              node_address: string;
              avg_cpu: number | null;
        ...
        rows[0] =
          "timestamp,node_address,rule_name,status,metric,value,threshold,message,acknowledged_at,resolved_at";
  ...
  node: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const from = input.metricsFrom
        ? new Date(input.metricsFrom)
      ...
      return {
        ...
        pagination: {
          ...
          nextPage:
            skip + metrics.length < metricsTotal ? input.metricsPage + 1 : null,
        },
      };
    }),
  ...
  network: publicProcedure.query(async ({ ctx }) => {
    const [totalNodes, activeNodes, versions, latestMetrics] =
      await Promise.all([
        ctx.db.node.count(),
    ...
    return {
      ...
      data: {
        ...
        aggregateMetrics: {
          ...
          totalStorageBytes: stats?.total_storage
            ? Number(stats.total_storage)
            : 0,
