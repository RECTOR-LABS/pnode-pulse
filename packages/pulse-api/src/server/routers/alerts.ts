...
const alertRuleInput = z.object({
  ...
  targetType: z
    .enum(["ALL_NODES", "SPECIFIC_NODES", "BOOKMARKED"])
    .default("ALL_NODES"),
...
export const alertsRouter = createTRPCRouter({
  ...
  createRule: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (
        input.rule.targetType === "SPECIFIC_NODES" &&
        input.rule.nodeIds.length === 0
      ) {
        throw new TRPCError({
          code: "BAD_REQUEST",
  ...
  history: publicProcedure
    .input(
      z.object({
        sessionId: z.string(),
        status: z
          .enum(["ACTIVE", "ACKNOWLEDGED", "RESOLVED", "SUPPRESSED", "ALL"])
          .default("ALL"),
