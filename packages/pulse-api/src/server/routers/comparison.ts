...
export const comparisonRouter = createTRPCRouter({
  ...
  versionStatus: publicProcedure.query(async ({ ctx }) => {
    ...
    const versionDistribution: Record<
      string,
  ...
  portfolioVersionStatus: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const onLatest = portfolioNodes.filter(
  ...
  underperformers: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const latestVersion = sortVersionsDesc(
      ...
      for (const pn of portfolio.nodes) {
        ...
        if (metric) {
          ...
          if (
            ...
            metric.cpuPercent > networkAvg.avg_cpu * 2
          ) {
            issues.push({
              metric: "cpu",
              ...
              severity:
                metric.cpuPercent > networkAvg.avg_cpu * 3
                  ? "critical"
                  : "warning",
          ...
          if (
            metric.uptime !== null &&
            metric.uptime < networkAvg.avg_uptime * 0.5
          ) {
            issues.push({
              metric: "uptime",
              ...
              severity:
                metric.uptime < networkAvg.avg_uptime * 0.25
                  ? "critical"
                  : "warning",
        ...
        if (issues.length > 0) {
          ...
          const score = Math.max(
            0,
            100 - criticalCount * 30 - warningCount * 15 - infoCount * 5,
          );
  ...
  compareNodes: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const comparison = nodes.map((node) => {
        ...
        const ramPercent =
          m?.ramUsed && m?.ramTotal && m.ramTotal > BigInt(0)
  ...
  peerHealth: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const stalePercent =
        peers.length > 0 ? Math.round((stalePeers / peers.length) * 100) : 0;
      ...
      const avgPeerCount =
        networkPeerAvg.length > 0
          ? networkPeerAvg.reduce((sum, n) => sum + n._count.id, 0) /
            networkPeerAvg.length
          : 0;
      ...
      return {
        ...
        versionDistribution: Object.entries(versionCounts).map(
          ([version, count]) => ({
            version,
            count,
  ...
  recommendations: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const highRamNodes: Array<{
        id: number;
        address: string;
        value: number;
      }> = [];
      const highCpuNodes: Array<{
        id: number;
        address: string;
        value: number;
      }> = [];
      const outdatedNodes: Array<{
        id: number;
        address: string;
        version: string;
      }> = [];
      const lowPeerNodes: Array<{
        id: number;
        address: string;
        count: number;
      }> = [];
      ...
      if (offlineNodes.length > 0) {
        recommendations.push({
          ...
          description:
            "These nodes are not responding. Check network connectivity and node status.",
      ...
      if (highRamNodes.length > 0) {
        recommendations.push({
          id: "high-ram",
          type: "optimization",
          severity: highRamNodes.some((n) => n.value > 90)
            ? "critical"
            : "warning",
          title: `High RAM usage on ${highRamNodes.length} node${highRamNodes.length > 1 ? "s" : ""}`,
          description:
            "RAM usage above 80% may impact stability. Consider increasing RAM or optimizing storage settings.",
      ...
      if (highCpuNodes.length > 0) {
        recommendations.push({
          id: "high-cpu",
          type: "optimization",
          severity: highCpuNodes.some((n) => n.value > 90)
            ? "critical"
            : "warning",
          title: `High CPU usage on ${highCpuNodes.length} node${highCpuNodes.length > 1 ? "s" : ""}`,
          description:
            "CPU usage above 70% may indicate performance issues or resource contention.",
