...
export const apiKeysRouter = createTRPCRouter({
  ...
  get: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const totalRequests = key.usageLogs.reduce(
        (sum, log) => sum + log.requestCount,
        0,
      );
      const totalErrors = key.usageLogs.reduce(
        (sum, log) => sum + log.errorCount,
        0,
      );
      const avgResponseTime =
        key.usageLogs.length > 0
          ? key.usageLogs.reduce((sum, log) => sum + log.totalResponseMs, 0) /
            totalRequests
          : 0;
  ...
  usage: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const timeSeries = usageLogs.map((log) => ({
        ...
        avgLatency:
          log.requestCount > 0
            ? Math.round(log.totalResponseMs / log.requestCount)
            : 0,
      }));
      ...
      return {
        ...
        summary: {
          ...
          avgLatency:
            totals.requests > 0
              ? Math.round(totals.totalMs / totals.requests)
              : 0,
        },
        timeSeries,
