...
export const HEALTH_WEIGHTS = {
  ...
  cpu: 0.2, // 20% - Performance indicator
  ram: 0.2, // 20% - Resource efficiency
  connectivity: 0.15, // 15% - Network participation
  version: 0.1, // 10% - Up-to-date software
} as const;
 ...
 */
function calculateUptimeScore(uptime: number): {
  score: number;
  status: string;
} {
 ...
 */
function calculateCpuScore(cpuPercent: number): {
  score: number;
  status: string;
} {
 ...
 */
function calculateRamScore(ramPercent: number): {
  score: number;
  status: string;
} {
 ...
 */
function calculateConnectivityScore(peerCount: number | undefined): {
  score: number;
  status: string;
} {
  ...
  return {
    ...
    status:
      peerCount === 0 ? "Poor - No peers" : `Poor - Only ${peerCount} peer(s)`,
  };
}
...
function calculateVersionScore(
  ...
  if (majorDiff === 0 && minorDiff === 0) {
    ...
    return {
      score: 70,
      status: `${patchDiff} patches behind (v${nodeVersion})`,
    };
  }
  ...
  if (majorDiff === 0 && minorDiff > 1) {
    return {
      score: 40,
      status: `${minorDiff} minor versions behind (v${nodeVersion})`,
    };
  }
  ...
  return {
    score: 0,
    status: `${majorDiff} major versions behind (v${nodeVersion})`,
  };
}
...
export function calculateNodeHealth(
  ...
  const version = calculateVersionScore(
    metrics.version,
    networkStats.latestVersion,
  );
  ...
  const outliers = {
    cpu: categorizeValue(
      metrics.cpuPercent,
      networkStats.avgCpu,
      networkStats.cpuStdDev,
    ),
    ram: categorizeValue(
      metrics.ramPercent,
      networkStats.avgRam,
      networkStats.ramStdDev,
    ),
    uptime: categorizeValue(
      metrics.uptime,
      networkStats.avgUptime,
      networkStats.uptimeStdDev,
    ),
  };
 ...
 */
export function calculateNetworkHealth(nodeScores: HealthScore[]): {
  avgScore: number;
  grade: HealthGrade;
