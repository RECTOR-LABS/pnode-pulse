...
export function analyzePeerConnectivity(
  ...
  if (activePeers.length === 0) {
    recommendations.push({
      ...
      description:
        "This node has no active peer connections, making it isolated from the network.",
      action:
        "Check network configuration, firewall rules, and ensure port 9001 is accessible.",
    });
  } else if (activePeers.length < THRESHOLDS.minHealthyPeers) {
    recommendations.push({
      id: "low-peers",
      ...
      action:
        "Consider improving network visibility or checking connectivity settings.",
    });
  } else if (activePeers.length < THRESHOLDS.optimalPeers) {
    recommendations.push({
      id: "moderate-peers",
      ...
      action:
        "Peer count is acceptable but could be improved for better network participation.",
    });
  }
  ...
  if (stalePeers.length > inactivePeers.length * 0.5 && stalePeers.length > 5) {
    recommendations.push({
      ...
      action:
        "Consider clearing stale peer entries to improve connection efficiency.",
    });
  }
  ...
  if (peerVersions.length === 1 && peers.length >= 5) {
    recommendations.push({
      ...
      action:
        "Not necessarily a problem, but diverse connections can improve resilience.",
    });
  }
...
export function analyzeNetworkConnectivity(
  ...
  if (isolatedRatio > 0.2) {
    recommendations.push({
      ...
      action:
        "Investigate network-wide connectivity issues. Check DNS, firewall configurations.",
    });
    ...
    recommendations.push({
      ...
      action:
        "Work with operators of isolated nodes to improve their network configuration.",
    });
  }
  ...
  if (wellConnectedRatio < 0.3) {
    recommendations.push({
      ...
      action:
        "Encourage operators to improve node visibility and network access.",
    });
  }
