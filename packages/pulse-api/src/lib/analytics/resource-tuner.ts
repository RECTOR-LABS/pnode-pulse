...
export function generateTuningRecommendations(
  ...
  if (metrics.cpuPercent >= THRESHOLDS.cpu.veryHigh) {
    recommendations.push({
      ...
      action:
        "Investigate CPU-intensive processes. Consider upgrading hardware or optimizing workloads.",
      metric: {
    ...
    recommendations.push({
      ...
      action:
        "Monitor CPU trends. Consider load balancing or resource optimization.",
      metric: {
        current: metrics.cpuPercent,
    ...
    metrics.cpuPercent < THRESHOLDS.cpu.low &&
    ...
    recommendations.push({
      ...
      action:
        "Consider rightsizing the node or adding additional pNode workloads.",
      metric: {
        current: metrics.cpuPercent,
  ...
  if (metrics.ramPercent >= THRESHOLDS.ram.veryHigh) {
    recommendations.push({
      ...
      action:
        "Add more RAM or identify memory-intensive processes. Consider enabling swap if not configured.",
      metric: {
    ...
    recommendations.push({
      ...
      action:
        "Monitor memory trends. Consider adding RAM or optimizing memory usage.",
      metric: {
        current: metrics.ramPercent,
    ...
    metrics.ramPercent < THRESHOLDS.ram.low &&
    metrics.uptime > 86400
  ) {
    recommendations.push({
  ...
  if (metrics.uptime < THRESHOLDS.uptime.unstable) {
    recommendations.push({
      ...
      action:
        "Investigate cause of restart. Check logs for errors or crashes. Ensure proper system configuration.",
      metric: {
    ...
    recommendations.push({
      ...
      action:
        "Monitor for unexpected restarts. Review system logs for warnings.",
      metric: {
        current: metrics.uptime,
  ...
  if (metrics.peerCount !== undefined) {
    if (metrics.peerCount <= THRESHOLDS.peers.veryLow) {
      recommendations.push({
        ...
        action:
          "Check network configuration and firewall rules. Ensure port 9001 is accessible.",
        metric: {
      ...
      recommendations.push({
        ...
        action:
          "Monitor peer discovery. Consider checking network connectivity.",
        metric: {
          current: metrics.peerCount,
  ...
  if (metrics.version && latestVersion) {
    const currentParts = metrics.version
      .replace(/^v/, "")
      .split(".")
      .map(Number);
...
export function summarizeRecommendations(
  ...
  const issueCount: Record<
    string,
    { count: number; priority: RecommendationPriority }
  > = {};
