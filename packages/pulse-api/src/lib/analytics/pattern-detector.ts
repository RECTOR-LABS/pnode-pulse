...
export function getExpectedValue(
  ...
  return hourlyValue * 0.6 + dowValue * 0.3 + baseline.stats.mean * 0.1;
}
...
export function calculateDeviation(
  ...
  if (
    ...
    expectedValue < DEVIATION_THRESHOLDS.minBaseline
  ) {
    return {
      deviation: 0,
  ...
  // Determine type
  const deviationType: DeviationType =
    deviation > 0 ? "above_baseline" : "below_baseline";
...
export function detectSuddenChange(
  ...
  const changePercent =
    recentMean > 0 ? ((currentValue - recentMean) / recentMean) * 100 : 0;
 ...
 */
export function summarizeDeviations(deviations: PatternDeviation[]): {
  total: number;
